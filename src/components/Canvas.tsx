...
function elementStyle(element: SlideElement, selected: boolean): CSSProperties {
  return {
    position: 'absolute',
    left: element.x,
    top: element.y,
...
export function Canvas({ slide, selectedElementId, onSelectElement, onMoveElement, onResizeElement }: CanvasProps) {
  ...
  return (
    <div className="canvas-shell">
      <div className="slide-canvas" style={{ width: slide.width, height: slide.height, ...backgroundStyle(slide) }} onPointerDown={() => onSelectElement(null)}>
        {sortedElements.map((element) => {
          const selected = selectedElementId === element.id;
          if (element.type === 'text') {
            return <div key={element.id} className="slide-element text-element" style={{ ...elementStyle(element, selected), ...element.style }} onPointerDown={(event: any) => startDrag(event, element)}>{element.text}{selected && <button className="resize-handle" aria-label="Resize selected element" onPointerDown={(event: any) => startResize(event, element)} />}</div>;
          }
          if (element.type === 'shape') {
            return <div key={element.id} className="slide-element shape-element" style={{ ...elementStyle(element, selected), background: element.style.fill, border: `${element.style.strokeWidth ?? 0}px solid ${element.style.stroke ?? 'transparent'}`, borderRadius: element.style.borderRadius, opacity: element.style.opacity }} onPointerDown={(event: any) => startDrag(event, element)}>{selected && <button className="resize-handle" aria-label="Resize selected element" onPointerDown={(event: any) => startResize(event, element)} />}</div>;
          }
          return <div key={element.id} className="slide-element html-element" style={elementStyle(element, selected)} onPointerDown={(event: any) => startDrag(event, element)}><iframe title="Imported HTML" sandbox="allow-same-origin" srcDoc={element.html} />{selected && <button className="resize-handle" aria-label="Resize selected element" onPointerDown={(event: any) => startResize(event, element)} />}</div>;
        })}
      </div>
    </div>
  );
