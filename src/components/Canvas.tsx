...
export function Canvas({ slide, selectedElementId, onSelectElement, onMoveElement }: CanvasProps) {
      ...
      <div className="slide-canvas" style={{ width: slide.width, height: slide.height, ...backgroundStyle(slide) }} onPointerDown={() => onSelectElement(null)}>
        {sortedElements.map((element) => {
          ...
          if (element.type === 'text') {
            return <div key={element.id} className="slide-element text-element" style={{ ...elementStyle(element, selected), ...element.style }} onPointerDown={(event: any) => startDrag(event, element)}>{element.text}</div>;
          }
          ...
          return <div key={element.id} className="slide-element html-element" style={elementStyle(element, selected)} onPointerDown={(event: any) => startDrag(event, element)}><iframe title="Imported HTML" sandbox="allow-same-origin" srcDoc={element.html} /></div>;
        })}
      </div>
    </div>
  );
