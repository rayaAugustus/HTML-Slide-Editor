...
import { importHtmlAsSandbox } from './core/importer/htmlImporter';
import './styles.css';
...
export default function App() {
  ...
  const importHtmlFile = async (file: File) => {
    ...
    const slide = importHtmlAsSandbox(html);
    updateDeck((previous) => ({ ...previous, slides: [...previous.slides, slide] }));
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        <Canvas slide={currentSlide} selectedElementId={selectedElementId} onSelectElement={setSelectedElementId} onMoveElement={moveElement} />
        <aside className="inspector"><h2>State</h2>{selectedElement ? <p>Selected: <strong>{selectedElement.type}</strong><br />x: {selectedElement.x}, y: {selectedElement.y}</p> : <p>No element selected.</p>}<button onClick={deleteSelected} disabled={!selectedElement}>Delete selected</button><pre>{JSON.stringify(currentSlide, null, 2)}</pre></aside>
      </section>
    </main>
  );
}
